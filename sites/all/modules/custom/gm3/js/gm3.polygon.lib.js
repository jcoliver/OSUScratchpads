 ...
 *               Enjoy guys:) <br/>Special thanks
 *               <code>Jan Pieter Waagmeester jieter@jpwaag.com</code> for the
 *               idea of using the library google.maps.geometry , which performs
 *               spherical linear interpolation between the two locations.
 *               <br/>Special thanks
 *               <code>James Ratcliff falazar@yahoo.com</code> for the idea of
 *               extending my previous script polylineEdit.js to polygonEdit.js
 */
/**
 * @name google
...
if(typeof google != 'undefined' && typeof (google.maps.Polygon.prototype.runEdit) === "undefined") {
  ...
  google.maps.Polygon.prototype.runEdit = function(flag){
    ...
    var self = this;
    if(flag) {
      var imgGhostVertex = new google.maps.MarkerImage(Drupal.settings.gm3.settings.images.sprite, new google.maps.Size(11, 11), new google.maps.Point(0, 22), new google.maps.Point(6, 6));
      var imgGhostVertexOver = new google.maps.MarkerImage(Drupal.settings.gm3.settings.images.sprite, new google.maps.Size(11, 11), new google.maps.Point(0, 33), new google.maps.Point(6, 6));
      var ghostPath = new google.maps.Polygon({map: this.getMap(), strokeColor: this.strokeColor, strokeOpacity: 0.2, strokeWeight: this.strokeWeight});
      ...
      var createGhostMarkerVertex = function(point){
        if(point.marker.inex < self.getPath().getLength() - 1) {
          var markerGhostVertex = new google.maps.Marker({position: (typeof (google.maps.geometry) === "undefined") ? new google.maps.LatLng(point.lat() + 0.5 * (self.getPath().getAt(point.marker.inex + 1).lat() - point.lat()), point.lng() + 0.5 * (self.getPath().getAt(point.marker.inex + 1).lng() - point.lng())) : google.maps.geometry.spherical.interpolate(point, self.getPath().getAt(point.marker.inex + 1), 0.5), map: self.getMap(), icon: imgGhostVertex, draggable: true, raiseOnDrag: false});
          ...
          if(point.marker.inex === self.getPath().getLength() - 1) {
            var markerGhostVertex = new google.maps.Marker({position: (typeof (google.maps.geometry) === "undefined") ? new google.maps.LatLng(point.lat() + 0.5 * (self.getPath().getAt(0).lat() - point.lat()), point.lng() + 0.5 * (self.getPath().getAt(0).lng() - point.lng())) : google.maps.geometry.spherical.interpolate(point, self.getPath().getAt(0), 0.5), map: self.getMap(), icon: imgGhostVertex, draggable: true, raiseOnDrag: false});
            google.maps.event.addListener(markerGhostVertex, "mouseover", vertexGhostMouseOver);
            google.maps.event.addListener(markerGhostVertex, "mouseout", vertexGhostMouseOut);
            google.maps.event.addListener(markerGhostVertex, "drag", vertexGhostDrag);
            google.maps.event.addListener(markerGhostVertex, "dragend", vertexGhostDragEnd);
    ...
    var imgVertex = new google.maps.MarkerImage(Drupal.settings.gm3.settings.images.sprite, new google.maps.Size(11, 11), new google.maps.Point(0, 0), new google.maps.Point(6, 6));
    var imgVertexOver = new google.maps.MarkerImage(Drupal.settings.gm3.settings.images.sprite, new google.maps.Size(11, 11), new google.maps.Point(0, 11), new google.maps.Point(6, 6));
